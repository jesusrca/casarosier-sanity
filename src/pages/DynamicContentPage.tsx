...
export function DynamicContentPage() {
  ...
  // Establecer el número de WhatsApp específico de esta clase/workshop cuando se carga
  useEffect(() => {
    if (content?.whatsappNumber) {
      setPhoneNumber(content.whatsappNumber);
    }
    
    // Limpiar el número cuando se desmonta el componente (al salir de la página)
    return () => {
      setPhoneNumber(null);
    };
  }, [content, setPhoneNumber]);
  ...
  const handleConsultar = () => {
    ...
    // Usar el número de WhatsApp específico del contenido o el configurado globalmente
    const whatsappNumber = content.whatsappNumber || '34633788860';
